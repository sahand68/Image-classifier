...
def generate_argparse():
    ...
    parser.add_argument('--model_path', dest="model_path", action="store", default='mycheckpoint.pth.tar')
    parser.add_argument('--topk', type=int, default=5, dest = 'topk',help='number of top likey classes to plot')
    parser.add_argument('--label_path', dest = 'label_path', action = 'store' , default ='cat_to_name.json')
...
def predict(image_path, label_path,model_path,topk=5, device ='gpu'):
...
def main():
    ...
    probs, classes = predict(image_path, label_path, model_path,topk=5, device=device)
    print(probs, classes)
